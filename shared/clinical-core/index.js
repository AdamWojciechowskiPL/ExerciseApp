// ExerciseApp/shared/clinical-core/index.js

const ClinicalRulesCore = (() => {
    const contracts = typeof globalThis !== 'undefined' && globalThis.__ClinicalContracts
        ? globalThis.__ClinicalContracts
        : (typeof require === 'function' ? require('./contracts.js') : null);

    const DECISION_REASONS = contracts ? contracts.DECISION_REASONS : {
    ...
    const createClinicalRules = (deps = {}) => {
        ...
        const buildClinicalContext = (wizardData) => {
            ...
            const acuteGuard = computeAcuteGuard(data);
        ...
        const checkExerciseAvailability = (ex, ctx, options = {}) => {
            ...
            if (ctx.blockedIds?.has(ex.id)) return { allowed: false, reason: DECISION_REASONS.BLACKLISTED };
            if (!ignoreEquipment && !checkEquipment(ex, ctx.userEquipment)) return { allowed: false, reason: DECISION_REASONS.MISSING_EQUIPMENT };

            const exLevel = getField(ex, 'difficulty_level', 'difficultyLevel', 1) || 1;
            if (!ignoreDifficulty && ctx.difficultyCap && exLevel > ctx.difficultyCap) return { allowed: false, reason: DECISION_REASONS.TOO_HARD_CALCULATED };
            if (violatesRestrictions(ex, ctx)) return { allowed: false, reason: DECISION_REASONS.PHYSICAL_RESTRICTION };

            const mismatch = isDirectionalMismatch(ex, ctx.tolerancePattern);
            if (mismatch && (ctx.toleranceBias?.strength || 0) >= 1) return { allowed: false, reason: DECISION_REASONS.BIOMECHANICS_MISMATCH };

            if (strictSeverity && ctx.isSevere) {
                if (norm(getField(ex, 'spine_load_level', 'spineLoadLevel', 'low')) === 'high') return { allowed: false, reason: DECISION_REASONS.SEVERITY_FILTER };
                const kneeLoad = norm(getField(ex, 'knee_load_level', 'kneeLoadLevel', 'low'));
                if ((ctx.painFilters.has('knee') || ctx.painFilters.has('knee_anterior')) && kneeLoad === 'high') return { allowed: false, reason: DECISION_REASONS.SEVERITY_FILTER };

                const zones = ex.pain_relief_zones || ex.painReliefZones || [];
                if (!zones.some((z) => ctx.painZoneSet.has(z))) return { allowed: false, reason: DECISION_REASONS.SEVERITY_FILTER };
            }

            return mismatch ? { allowed: true, reason: DECISION_REASONS.DIRECTIONAL_BIAS } : { allowed: true, reason: null };
        };
