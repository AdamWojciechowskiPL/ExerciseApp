...
const { calculateFatigueProfile } = require('./_fatigue-calculator.js'); // US-06: Nowy import
...
// ============================================================================
// 3. WEIGHTING LOGIC (G2 SCORING PREP)
// ============================================================================
...
function multiplyMatching(weights, predicateFn, factor) {
    for (const cat of Object.keys(weights)) {
        if (predicateFn(cat)) { weights[cat] = weights[cat] * factor; }
    }
}
...
function buildDynamicCategoryWeights(exercises, userData, ctx) {
  ...
  if (painLocs.has('knee') || painLocs.has('knee_anterior')) {
    ...
    const tkeBoost = ctx.isSevere ? 1.2 : 1.5;
  ...
  if (painLocs.has('lumbar') || painLocs.has('low_back')) {
    ...
    const coreBoost = ctx.isSevere ? 1.7 : 2.0;
  ...
  if (painLocs.has('ankle') || painLocs.has('foot')) {
    ...
    boost(weights, 'balance_proprioception', 0.5);
  }
  ...
  if (diagnosis.has('piriformis') || painLocs.has('sciatica') || ctx.painFilters.has('sciatica')) {
    ...
    boost(weights, 'nerve_flossing', nfBoost);
  ...
  // 6. User Preferences & Goal Tuning
...
function varietyPenalty(ex, state, section) {
  ...
  if (isAnchor) {
      ...
      if (familyUsage < targetExposure) {
          p = 1.0;
      } else {
          p *= 1.0 / (1.0 + (familyUsage - targetExposure + 1) * 1.2);
      }
  } else {
...
// US-05: ANCHORS
function selectMicrocycleAnchors(candidates, userData, ctx, categoryWeights, phaseContext) {
    ...
    const scored = candidates.map(ex => ({
        ex,
        score: scoreExercise(ex, 'main', userData, ctx, categoryWeights, mockState, new Set(), phaseContext)
...
function pickExerciseForSection(section, candidates, userData, ctx, categoryWeights, state, painZoneSet, extraFilterFn = null, phaseContext = null) {
  ...
  if (section === 'main') {
      const primaryScore = scoreExercise(picked, section, userData, ctx, categoryWeights, state, painZoneSet, phaseContext);
      const primaryFamily = deriveFamilyKey(picked);

      const potentialAlts = filtered.filter(ex =>
          ex.id !== picked.id && deriveFamilyKey(ex) === primaryFamily
      );

      let altCandidate = null;
      if (potentialAlts.length > 0) {
          potentialAlts.sort((a, b) => {
              const sA = scoreExercise(a, section, userData, ctx, categoryWeights, state, painZoneSet, phaseContext);
              const sB = scoreExercise(b, section, userData, ctx, categoryWeights, state, painZoneSet, phaseContext);
              return sB - sA;
          });
      ...
      if (altCandidate) {
          const altScore = scoreExercise(altCandidate, section, userData, ctx, categoryWeights, state, painZoneSet, phaseContext);
          if (altScore >= primaryScore * 0.95) {
              picked.alternatives = [JSON.parse(JSON.stringify(altCandidate))];
          }
      }
  }
...
function getPhaseRestFactor(phaseConfig, section = 'main') {
    ...
    if (section === 'warmup' || section === 'cooldown') {
        phaseFactor = Math.min(phaseFactor, 1.0);
    }
    return phaseFactor;
}
...
function estimateExerciseDurationSeconds(exEntry, userData, paceMap, effectiveRestFactor) {
  ...
  if (sets > 1) {
      restTimePerSet = getRestAfterExercise(exEntry, effectiveRestFactor);
...
function estimateSessionDurationSeconds(session, userData, paceMap, phaseContext) {
  ...
  for (const section of sections) {
      ...
      for (let i = 0; i < exercises.length; i++) {
          ...
          const isLastInSession = (section === 'cooldown' && i === exercises.length - 1) ||
                                  (section === 'main' && i === exercises.length - 1 && (!session.cooldown || session.cooldown.length === 0)) ||
                                  (section === 'warmup' && i === exercises.length - 1 && (!session.main || session.main.length === 0) && (!session.cooldown || session.cooldown.length === 0));

          if (!isLastInSession) {
              total += getRestAfterExercise(ex, userRestFactor);
          }
      }
  }
  return total;
...
function logFinalSessionBreakdown(session, userData, paceMap, phaseContext) {
    ...
    all.forEach((ex, i) => {
        ...
        if (i < all.length - 1) {
            const rbe = getRestAfterExercise(ex, userRestFactor);
...
// ============================================================================
// 7. ROLLING PLAN BUILDER (US-06 Updated)
// ============================================================================
...
function buildRollingPlan(candidates, categoryWeights, userData, ctx, userId, historyMap, preferencesMap, paceMap, fatigueProfile, rpeData, progressionMap, phaseContext) {
    ...
    const { anchorFamilies, targetExposure } = selectMicrocycleAnchors(candidates, userData, ctx, categoryWeights, phaseContext);
    ...
    const plan = {
        ...
        meta: {
            ...
            fatigueProfile: fatigueProfile, // US-06: Store full profile for telemetry/debugging
        ...
        if (isScheduled && !isForcedRest) {
            ...
            ['warmup', 'main', 'cooldown'].forEach(sec => {
                ...
                for (let i = 0; i < counts[sec]; i++) {
                    ...
                    if (ex) {
                        // US-06: Use calibrated filter threshold from profile
                        const loopFatigueState = fatigueScore >= fatigueProfile.fatigueThresholdFilter ? 'fatigued' : 'fresh';
                        const rx = prescribeForExercise(ex, sec, userData, ctx, categoryWeights, loopFatigueState, targetMin, rpeData.volumeModifier, phaseContext);
            ...
            // US-06: Use calibrated filter threshold for volume expansion limits
            const loopFatigueState = fatigueScore >= fatigueProfile.fatigueThresholdFilter ? 'fatigued' : 'fresh';
            expandSessionToTarget(session, candidates, userData, ctx, categoryWeights, sState, pZones, paceMap, loopFatigueState, targetMin, rpeData.volumeModifier, phaseContext);
            ...
            fatigueScore = Math.max(0, fatigueScore - 50); // Rest day recovery
...
exports.handler = async (event) => {
  ...
  try {
    const [eR, bR, pR, hR, sR, recentSessionsR, oR, fatigueProfile, settingsR] = await Promise.all([
      ...
      calculateFatigueProfile(client, userId), // US-06: New Fatigue Calculation
    ...
    // US-06: Create comprehensive safety context with calibrated thresholds
    const safetyCtx = {
        isSeverePain: ctx.isSevere,
        fatigueScore: fatigueProfile.fatigueScoreNow,
        fatigueThresholdEnter: fatigueProfile.fatigueThresholdEnter,
        fatigueThresholdExit: fatigueProfile.fatigueThresholdExit,
        monotony7d: fatigueProfile.monotony7d,
        strain7d: fatigueProfile.strain7d,
    ...
    console.log(`[PlanGen] User: ${userId}, Phase: ${phaseContext.phaseId} (Override: ${phaseContext.isOverride}), Fatigue: ${fatigueProfile.fatigueScoreNow}/${fatigueProfile.fatigueThresholdEnter}`);

    const cWeights = buildDynamicCategoryWeights(exercises, userData, ctx);
    
    // US-06: Use calibrated filter threshold from profile
    const fatigueStateForFilter = fatigueProfile.fatigueScoreNow >= fatigueProfile.fatigueThresholdFilter ? 'fatigued' : 'fresh';
    ...
    // US-06: Pass fatigueProfile to buildRollingPlan
